...
export default function Home() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch("https://api.remove.bg/v1.0/removebg", {
        method: "POST",
        headers: { "X-Api-Key": "pBzBsLfZKHNxiiTEvbX4seU2" },
        body: (() => { const fd = new FormData(); fd.append("image_file", photoFile); fd.append("size", "auto"); return fd; })(),
      });
      ...
      const prompt = `Epic 3D anamorphic billboard advertisement in ${selBg?.label}, giant photorealistic product breaking out of the billboard frame into real world, dramatic lighting, cinematic, hyperrealistic, Times Square style viral advertisement${slogan ? `, text "${slogan}"` : ""}, professional CGI render, 8k`;

      const res = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ imageUrl: base64, prompt }),
      });
    ...
    <div style={{ minHeight: "100vh", background: "#07070e", color: "#fff", fontFamily: "'Inter',system-ui,sans-serif", padding: "0 0 60px" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .inp{width:100%;background:rgba(255,255,255,.06);border:1.5px solid rgba(255,255,255,.1);border-radius:13px;padding:14px 16px;color:#fff;font-size:15px;font-family:inherit;outline:none;transition:border-color .2s;}
        .inp:focus{border-color:rgba(236,72,153,.55);}
        .inp::placeholder{color:rgba(255,255,255,.3);}
        .btn{width:100%;padding:17px;background:linear-gradient(135deg,#ec4899,#f97316);border:none;border-radius:14px;color:#fff;font-size:16px;font-weight:800;cursor:pointer;font-family:inherit;transition:transform .15s;letter-spacing:.5px;}
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 18px" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div
            ...
            style={{ border: "2px dashed rgba(255,45,120,.35)", borderRadius: 14, padding: photo ? 0 : 32, textAlign: "center", cursor: "pointer", overflow: "hidden", minHeight: 90, display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s" }}
            onMouseEnter={e => (e.currentTarget.style.borderColor = "rgba(255,45,120,.7)")}
            onMouseLeave={e => (e.currentTarget.style.borderColor = "rgba(255,45,120,.35)")}
          >
            {photo
              ...
              : <div>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,.22)", marginTop: 4 }}>JPG · PNG · WEBP</div>
                </div>}
          </div>
        ...
        <button className="btn" onClick={generate} disabled={loading}>
          {loading ? `⚡ Generando… ${Math.round(progress)}%` : "⚡ Generar Billboard 3D"}
        </button>
